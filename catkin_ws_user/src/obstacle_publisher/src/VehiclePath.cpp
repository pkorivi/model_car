...
namespace fub_obstacle_publisher{
  ...
  */
  ...
  void VehiclePath::transformToXYandFrenet(){
    size_t number_of_pts = xy_way_points_.size();
    ...
    if(number_of_pts>0){
      ...
      size_t i;
      for ( i = 0; i < xy_way_points_.size(); i++) {
        tf::Point temp_pt = {xy_way_points_[i][0],xy_way_points_[i][1],0};
