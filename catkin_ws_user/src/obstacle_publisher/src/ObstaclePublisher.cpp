...
namespace fub_obstacle_publisher{
  ...
  void ObstaclePublisher::onInit(){
    ...
    int obst_id =0;
    // obst_start, obst_end, obst_d, obst_vel, obst_yaw , obst_id
    //Dynamic
    obst_to_publish.push_back(obstacle_def(1,5,0.17,0.5,0,obst_id++));
    //Static
    obst_to_publish.push_back(obstacle_def(2.5,2.5,0.17,0.0,0,obst_id++));
  ...
  void ObstaclePublisher::callbackTimer(const ros::TimerEvent &){
      ...
      for (size_t i = 0; i < obst_to_publish.size(); i++) {
        ...
        //Obstacle odometry
        geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromYaw(obst_to_publish[i].obst_yaw);
